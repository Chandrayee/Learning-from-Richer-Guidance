...
if __name__=='__main__':
	...
	#train_phis = np.load('optmdfs.npy')
	train_phis = np.load('trialdata2_high.npy')
	...
	with open(fn, 'rb') as out:
		X = pickle.load(out)
	#ind = np.load('deleteindicesfromdata3.npy')
	#X = np.delete(X, ind, axis = 0)
	array = np.array([np.linalg.norm(k) for k in train_phis])
	indices = []
	for i in range(len(array)):
		if array[i] < 2.0:
			indices.append(i)
	keepindices = np.delete(np.arange(0,len(X)), indices)
	train_phis = train_phis[keepindices]
	X = X[keepindices]
